...
const AddRestaurant = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await axios.get("https://food-bot-backend.onrender.com/api/getprod");
  ...
  let del=(pid)=>{
    console.log(pid)
    axios.delete(`https://food-bot-backend.onrender.com/auth/delprod/${pid}`,{"headers":{"Authorization":obj.state.token,"uid":obj.state._id}}).then((res)=>{
          ...
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product) => (
              <div key={product._id} className="restaurant-card">
                <img
                  src={`https://food-bot-backend.onrender.com/prdimg/${product.pimg}`}
                  alt={product.name}
                />
